...
from utils.utils_metrics_volker import compute_mIoU, show_results
import glob
import numpy as np
import torch
import os
...
def cal_miou(test_dir="./images/cracks/Test_Images",
             pred_dir="./images/cracks/results", gt_dir="./images/cracks/Test_Labels", miou_out_path="",
             model=""):
    # ---------------------------------------------------------------------------#
    ...
    if miou_mode == 0 or miou_mode == 1:
        ...
        # 加载模型参数
        net.load_state_dict(torch.load(model, map_location=device)) # todo
        # 测试模式
        net.eval()
...
if __name__ == '__main__':
    dataset = "Volker_aug2"
    model = f'unet_{dataset}_best_model.pth'
    test_dir = f"datasets/seg/{dataset}/test/images"
    pred_dir = f"datasets/seg/{dataset}/unet/unet_images"
    gt_dir = f"datasets/seg/{dataset}/test/masks"
    miou_out_path = f"datasets/seg/{dataset}/unet/results/"
    cal_miou(test_dir, pred_dir, gt_dir, miou_out_path, model)
